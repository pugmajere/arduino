...
#define increaseWindow 30 // Increase to increase sensitivity

// Then we track the number of times that our measured signal has decreased 
// within this window.  
#define pureIncreaseThreshold 5 // Decrease this to increase sensitivity
    ...
    if (isTouched && switchedCount == 0) { 
      ...
      for (int chan = 0; chan < channelCount; chan++) {
        if (whereTouched[chan]) { 
          ...
          int chan_to_adjust = (chan + 1) % sizeof(whereTouched);
          glow[chan_to_adjust] = !glow[chan_to_adjust];
        }
      }
